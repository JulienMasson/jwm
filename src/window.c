...
xcb_window_t window_create(uint16_t x, uint16_t y, uint16_t width, uint16_t height)
{
	...
	uint32_t values[3] = { screen->black_pixel, 1, XCB_EVENT_MASK_EXPOSURE | XCB_EVENT_MASK_SUBSTRUCTURE_NOTIFY};
...
void window_set_focus(xcb_window_t win)
{
	...
	xcb_change_property(conn, XCB_PROP_MODE_REPLACE, screen->root, ewmh->_NET_ACTIVE_WINDOW, XCB_ATOM_WINDOW, 32, 1, &win);
	xcb_flush(conn);
}
...
bool window_get_geom(xcb_window_t win, int16_t *x, int16_t *y,
...
bool window_check_type(xcb_window_t win)
{
	...
	xcb_get_property_cookie_t cookie = xcb_ewmh_get_wm_window_type(ewmh, win);

	if (xcb_ewmh_get_wm_window_type_reply(ewmh, cookie, &win_type, NULL) == 1) {
...
void window_get_limits_size(xcb_window_t win, uint16_t *max_width, uint16_t *max_height,
			    uint16_t *min_width, uint16_t *min_height)
{
	xcb_size_hints_t hints;
	xcb_get_property_cookie_t cookie = xcb_icccm_get_wm_normal_hints_unchecked(conn, win);
...
bool window_get_pointer(const xcb_window_t *win, int16_t *x, int16_t *y)
{
	...
	xcb_query_pointer_reply_t *pointer = xcb_query_pointer_reply(conn, cookie, 0);
	...
	if (xcb_icccm_get_wm_protocols_reply(conn, cookie, &protocols, NULL) == 1) {
		for (i = 0; i < protocols.atoms_len; i++)
			if (protocols.atoms[i] == atom_get(wm_delete_window)) {
				...
				xcb_send_event(conn, false, win, XCB_EVENT_MASK_NO_EVENT, (char *)&ev);
				use_delete = true;
				break;
			}
		xcb_icccm_get_wm_protocols_reply_wipe(&protocols);
	}
	if (!use_delete) xcb_kill_client(conn, win);
	xcb_flush(conn);
}
